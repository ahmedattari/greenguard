...
def get_sensor_data():
    url = "http://10.33.239.1/sensors"  # Change to your ESP32 IP
    try:
...
def update_sensor_nodes(sensor_data):
    if sensor_data:
        # Update DHT11 node (temperature and humidity)
...
def logout():
    session.clear()
    return redirect(url_for('home'))  # or url_for('auth_page') if you have a separate auth page
...
def signup():
    ...
    if user_node:
        ...
        moisture_node = Node("sensor", name="Moisture", user_id=user_node.identity, Soil_Moisture="0")
        mq135_node = Node("sensor", name="MQ135", user_id=user_node.identity, Air_Quality="0")
